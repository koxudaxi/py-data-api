...
def test_generated_fields_first() -> None:
    assert Result(generated_fields=[1, 2, 3]).generated_fields_first == 1
    assert Result(generated_fields=[1.1, 2, 3]).generated_fields_first == 1.1
    assert Result(generated_fields=['abc', 2, 3]).generated_fields_first == 'abc'


def test_generated_fields_first_empty() -> None:
    assert Result(generated_fields=[]).generated_fields_first is None
...
def test_execute_select(mocked_client, mocker) -> None:
    ...
    assert data_api.execute("select * from pets") == [[1, 'cat']]
    assert mocked_client.execute_statement.call_args == mocker.call(
        continueAfterTimeout=True,
        database='test',
        includeResultMetadata=False,
...
def test_execute_select_query(mocked_client, mocker) -> None:
    ...
    assert data_api.execute(Query(Users).filter(Users.id == 1)) == [[1, 'cat']]
...
def test_transaction(mocked_client):
    @transaction(
        resource_arn='dummy', secret_arn='dummy', database='test', client=mocked_client
    )
