...
from pydataapi.pydataapi import convert_value, generate_sql, create_sql_parameters, DataAPI, Result, transaction
...
class TestDataAPI(TestCase):
    ...
    def test_with_statement(self) -> None:
        with patch('boto3.client') as mock_client:
            ...
            with DataAPI(resource_arn='dummy', secret_arn='dummy', database='test', client=mock_client) as data_api:
                mock_client.begin_transaction.assert_called_once_with(database='test', resourceArn='dummy', secretArn='dummy')
            mock_client.commit_transaction.assert_called_once_with(resourceArn='dummy', secretArn='dummy', transactionId='abc')
    ...
    def test_with_statement_exception(self) -> None:
        with patch('boto3.client') as mock_client:
            ...
            mock_client.rollback_transaction.assert_called_once_with(resourceArn='dummy', secretArn='dummy', transactionId='abc')
    ...
    def test_with_statement_custom_exception(self) -> None:
        ...
        with patch('boto3.client') as mock_client:
            ...
            with self.assertRaises(CustomError):
                with DataAPI(resource_arn='dummy', secret_arn='dummy', database='test', client=mock_client,
    ...
    def test_begin(self) -> None:
        with patch('boto3.client') as mock_client:
            mock_client.begin_transaction.return_value = {'transactionId': 'abc'}
            data_api = DataAPI(resource_arn='dummy', secret_arn='dummy', database='test', client=mock_client)
            self.assertEqual(data_api.begin(schema='schema'), 'abc')
            self.assertEqual(mock_client.begin_transaction.call_args,
    ...
    def test_commit(self) -> None:
        with patch('boto3.client') as mock_client:
            ...
            self.assertEqual(mock_client.commit_transaction.call_args,
    ...
    def test_rollback(self) -> None:
        with patch('boto3.client') as mock_client:
            ...
            self.assertEqual(mock_client.rollback_transaction.call_args,
                             call(resourceArn='dummy', transactionId='abc', secretArn='dummy'))

    def test_execute_insert(self) -> None:
        with patch('boto3.client') as mock_client:
            mock_client.execute_statement.return_value = {'generatedFields': [],
            ...
            self.assertEqual(mock_client.execute_statement.call_args,
    ...
    def test_execute_select(self) -> None:
        with patch('boto3.client') as mock_client:
            mock_client.execute_statement.return_value = {'numberOfRecordsUpdated': 0,
                                                                       'records': [
    ...
    def test_execute_select_include_metadata(self) -> None:
        with patch('boto3.client') as mock_client:
            mock_client.execute_statement.return_value = { "columnMetadata": [
        {
        ...
        },
        {
            ...
            "typeName": "VARCHAR"
                                                                       ...
                                                                       'records': [
                                                                           [{'longValue': 1}, {'stringValue': 'cat'}]]}
            data_api = DataAPI(resource_arn='dummy', secret_arn='dummy', database='test', client=mock_client)
            self.assertEqual(data_api.execute("select * from pets", with_columns=True),
                             [{'id': 1, 'name': 'cat'}])
            self.assertEqual(mock_client.execute_statement.call_args,
                             call(continueAfterTimeout=True, database='test', includeResultMetadata=True,
                                  resourceArn='dummy', secretArn='dummy',
                                  sql='select * from pets'))
        ...
        with patch('boto3.client') as mock_client:
            mock_client.execute_statement.return_value = {'numberOfRecordsUpdated': 0,
                                                                       'records': [
                                                                           [{'longValue': 1}, {'stringValue': 'cat'}]]}
            data_api = DataAPI(resource_arn='dummy', secret_arn='dummy', database='test', client=mock_client)
            self.assertEqual(data_api.execute(Query(Users).filter(Users.id == 1)), [[1, 'cat']])
    ...
    def test_execute_insert_parameter_set(self) -> None:
        with patch('boto3.client') as mock_client:
            ...
            self.assertEqual(mock_client.batch_execute_statement.call_args,
...
class TestTransaction(TestCase):
    def test_transaction(self):
        with patch('boto3.client') as mock_client:
            @transaction(resource_arn='dummy', secret_arn='dummy', database='test', client=mock_client)
            ...
            mock_client.begin_transaction.return_value = {'transactionId': 'abc'}
            add_user(1, 'cat')
            mock_client.begin_transaction.assert_called_once_with(database='test', resourceArn='dummy',
                                                                      secretArn='dummy')
            mock_client.commit_transaction.assert_called_once_with(resourceArn='dummy', secretArn='dummy',
                                                                   transactionId='abc')
